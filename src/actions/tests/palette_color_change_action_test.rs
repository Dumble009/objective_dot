...
mod tests {
    ...
    fn run_undo_test() {
        ...
        let bc = palette.borrow().get_color((0, 0).into()).unwrap();
        let ac = ODColor::new(1, 1, 1);
        let mut action = PaletteColorChangeAction::new(palette.clone(), (0, 0).into(), bc, ac);
        assert!(action.run().is_ok());

        let actual = palette.borrow().get_color((0, 0).into()).unwrap();
        assert_eq!(ac, actual);
