#[cfg(test)]
mod test {
    use super::*;

    use crate::common::color::ODColor;
    use crate::mock::drawing_mock::DrawingMock;

    use std::cell::RefCell;
    use std::rc::Rc;

    const CANVAS_EDGE_SIZE: usize = 10;

    #[test]
    fn line_draw_test1() {
        let mut line = Line::new();
        let mut canvas = vec![vec![(0, 0).into(); CANVAS_EDGE_SIZE]; CANVAS_EDGE_SIZE];
        let canvas_size = (CANVAS_EDGE_SIZE, CANVAS_EDGE_SIZE);
        let drawing = Rc::new(RefCell::new(DrawingMock::new()));
        let palette = drawing.borrow().get_palette();
        assert!(palette
            .borrow_mut()
            .add_color(ODColor::new(0, 0, 0))
            .is_ok());
        assert!(palette.borrow_mut().select_color((1, 0).into()).is_ok());
        let grid = drawing.borrow().get_active_layer();
        assert!(grid.borrow_mut().set_grid_width(CANVAS_EDGE_SIZE).is_ok());
        assert!(grid.borrow_mut().set_grid_height(CANVAS_EDGE_SIZE).is_ok());

        // Start drawing a line
        let mouse_pos = (0, 0);
        assert!(line
            .on_mouse_down(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        // 表示上は塗られるが、この時点ではまだ本当のグリッドには反映されない
        assert_eq!(canvas[0][0], (1, 0));
        assert_eq!(
            drawing
                .borrow()
                .get_grid()
                .unwrap()
                .get_color(0, 0)
                .unwrap(),
            (0, 0)
        );

        // Dragging the mouse to draw the line
        let mouse_pos = (5, 5);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        for i in 0..6 {
            assert_eq!(canvas[i][i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, i)
                    .unwrap(),
                (0, 0)
            );
        }

        // Finish drawing the line
        let mouse_pos = (9, 9);
        let res = line.on_mouse_up(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos);
        assert!(res.is_ok());
        let option = res.unwrap();
        assert!(option.is_some());
        let mut action = option.unwrap();
        assert!(action.run().is_ok());

        // マウスを離した時点で本当のグリッドに反映される
        for i in 0..10 {
            assert_eq!(canvas[i][i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, i)
                    .unwrap(),
                (1, 0)
            );
        }
    }

    #[test]
    fn line_draw_test2() {
        let mut line = Line::new();
        let mut canvas = vec![vec![(0, 0).into(); CANVAS_EDGE_SIZE]; CANVAS_EDGE_SIZE];
        let canvas_size = (CANVAS_EDGE_SIZE, CANVAS_EDGE_SIZE);
        let drawing = Rc::new(RefCell::new(DrawingMock::new()));
        let palette = drawing.borrow().get_palette();
        assert!(palette
            .borrow_mut()
            .add_color(ODColor::new(0, 0, 0))
            .is_ok());
        assert!(palette.borrow_mut().select_color((1, 0).into()).is_ok());
        let grid = drawing.borrow().get_active_layer();
        assert!(grid.borrow_mut().set_grid_width(CANVAS_EDGE_SIZE).is_ok());
        assert!(grid.borrow_mut().set_grid_height(CANVAS_EDGE_SIZE).is_ok());

        // Start drawing a line
        let mouse_pos_start = (9, 9);
        assert!(line
            .on_mouse_down(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos_start)
            .is_ok());
        // 表示上は塗られるが、この時点ではまだ本当のグリッドには反映されない
        assert_eq!(canvas[9][9], (1, 0));
        assert_eq!(
            drawing
                .borrow()
                .get_grid()
                .unwrap()
                .get_color(9, 9)
                .unwrap(),
            (0, 0)
        );

        // Dragging the mouse to draw the line
        let mouse_pos_drag = (5, 5);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos_drag)
            .is_ok());
        for i in 0..5 {
            assert_eq!(canvas[9 - i][9 - i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(9 - i, 9 - i)
                    .unwrap(),
                (0, 0)
            );
        }

        // Finish drawing the line
        let mouse_pos_end = (0, 0);
        let res = line.on_mouse_up(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos_end);
        assert!(res.is_ok());
        let option = res.unwrap();
        assert!(option.is_some());
        let mut action = option.unwrap();
        assert!(action.run().is_ok());
        // マウスを離した時点で本当のグリッドに反映される
        for i in 0..10 {
            assert_eq!(canvas[i][i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, i)
                    .unwrap(),
                (1, 0)
            );
        }
    }

    #[test]
    fn line_draw_test3() {
        let mut line = Line::new();
        let mut canvas = vec![vec![(0, 0).into(); CANVAS_EDGE_SIZE]; CANVAS_EDGE_SIZE];
        let canvas_size = (CANVAS_EDGE_SIZE, CANVAS_EDGE_SIZE);
        let drawing = Rc::new(RefCell::new(DrawingMock::new()));
        let palette = drawing.borrow().get_palette();
        assert!(palette
            .borrow_mut()
            .add_color(ODColor::new(0, 0, 0))
            .is_ok());
        assert!(palette.borrow_mut().select_color((1, 0).into()).is_ok());
        let grid = drawing.borrow().get_active_layer();
        assert!(grid.borrow_mut().set_grid_width(CANVAS_EDGE_SIZE).is_ok());
        assert!(grid.borrow_mut().set_grid_height(CANVAS_EDGE_SIZE).is_ok());

        // Start drawing a line
        let mouse_pos = (9, 0);
        assert!(line
            .on_mouse_down(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        // 表示上は塗られるが、この時点ではまだ本当のグリッドには反映されない
        assert_eq!(canvas[0][9], (1, 0));
        assert_eq!(
            drawing
                .borrow()
                .get_grid()
                .unwrap()
                .get_color(9, 0)
                .unwrap(),
            (0, 0)
        );

        // Dragging the mouse to draw the line
        let mouse_pos = (5, 4);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        for i in 0..5 {
            println!("test {i}");
            assert_eq!(canvas[i][9 - i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(9 - i, i)
                    .unwrap(),
                (0, 0)
            );
        }

        // Finish drawing the line
        let mouse_pos = (0, 9);
        let res = line.on_mouse_up(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos);
        assert!(res.is_ok());
        let option = res.unwrap();
        assert!(option.is_some());
        let mut action = option.unwrap();
        assert!(action.run().is_ok());
        // マウスを離した時点で本当のグリッドに反映される
        for i in 0..10 {
            assert_eq!(canvas[i][9 - i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(9 - i, i)
                    .unwrap(),
                (1, 0)
            );
        }
    }

    #[test]
    fn line_draw_test4() {
        let mut line = Line::new();
        let mut canvas = vec![vec![(0, 0).into(); CANVAS_EDGE_SIZE]; CANVAS_EDGE_SIZE];
        let canvas_size = (CANVAS_EDGE_SIZE, CANVAS_EDGE_SIZE);
        let drawing = Rc::new(RefCell::new(DrawingMock::new()));

        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .add_color(ODColor::new(0, 0, 0))
            .is_ok());
        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .select_color((1, 0).into())
            .is_ok());

        assert!(drawing
            .borrow_mut()
            .set_grid_width(CANVAS_EDGE_SIZE)
            .is_ok());
        assert!(drawing
            .borrow_mut()
            .set_grid_height(CANVAS_EDGE_SIZE)
            .is_ok());

        // Start drawing a line
        let mouse_pos_start = (0, 9);
        assert!(line
            .on_mouse_down(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos_start)
            .is_ok());
        // 表示上は塗られるが、この時点ではまだ本当のグリッドには反映されない
        assert_eq!(canvas[9][0], (1, 0));
        assert_eq!(
            drawing
                .borrow()
                .get_grid()
                .unwrap()
                .get_color(0, 9)
                .unwrap(),
            (0, 0)
        );

        // Dragging the mouse to draw the line
        let mouse_pos_drag = (4, 5);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos_drag)
            .is_ok());
        for i in 0..5 {
            assert_eq!(canvas[9 - i][i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, 9 - i)
                    .unwrap(),
                (0, 0)
            );
        }

        // Finish drawing the line
        let mouse_pos_end = (9, 0);
        let res = line.on_mouse_up(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos_end);
        assert!(res.is_ok());
        let option = res.unwrap();
        assert!(option.is_some());
        let mut action = option.unwrap();
        assert!(action.run().is_ok());
        // マウスを離した時点で本当のグリッドに反映される
        for i in 0..10 {
            assert_eq!(canvas[9 - i][i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, 9 - i)
                    .unwrap(),
                (1, 0)
            );
        }
    }

    #[test]
    fn line_outside_canvas_test() {
        let mut line = Line::new();
        let mut canvas = vec![vec![(0, 0).into(); CANVAS_EDGE_SIZE]; CANVAS_EDGE_SIZE];
        let canvas_size = (CANVAS_EDGE_SIZE, CANVAS_EDGE_SIZE);
        let drawing = Rc::new(RefCell::new(DrawingMock::new()));

        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .add_color(ODColor::new(0, 0, 0))
            .is_ok());
        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .select_color((1, 0).into())
            .is_ok());

        assert!(drawing
            .borrow_mut()
            .set_grid_width(CANVAS_EDGE_SIZE)
            .is_ok());
        assert!(drawing
            .borrow_mut()
            .set_grid_height(CANVAS_EDGE_SIZE)
            .is_ok());

        let mouse_pos = (7, 10);
        assert!(line
            .on_mouse_down(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());

        let mouse_pos = (10, 7);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());

        for i in 0..2 {
            assert_eq!(canvas[9 - i][8 + i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(8 + i, 9 - i)
                    .unwrap(),
                (0, 0)
            );
        }

        let res = line.on_mouse_up(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos);
        assert!(res.is_ok());
        let option = res.unwrap();
        assert!(option.is_some());
        let mut action = option.unwrap();
        assert!(action.run().is_ok());
        for i in 0..2 {
            assert_eq!(canvas[9 - i][8 + i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(8 + i, 9 - i)
                    .unwrap(),
                (1, 0)
            );
        }
    }

    #[test]
    fn line_not_1x1_draw_test() {
        let mut line = Line::new();
        let mut canvas = vec![vec![(0, 0).into(); CANVAS_EDGE_SIZE]; CANVAS_EDGE_SIZE];
        let canvas_size = (CANVAS_EDGE_SIZE, CANVAS_EDGE_SIZE);
        let drawing = Rc::new(RefCell::new(DrawingMock::new()));

        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .add_color(ODColor::new(0, 0, 0))
            .is_ok());
        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .select_color((1, 0).into())
            .is_ok());

        assert!(drawing
            .borrow_mut()
            .set_grid_width(CANVAS_EDGE_SIZE)
            .is_ok());
        assert!(drawing
            .borrow_mut()
            .set_grid_height(CANVAS_EDGE_SIZE)
            .is_ok());

        let mouse_pos = (4, 4);
        assert!(line
            .on_mouse_down(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(
            drawing
                .borrow()
                .get_grid()
                .unwrap()
                .get_color(4, 4)
                .unwrap(),
            (0, 0)
        );

        let mouse_pos = (0, 1);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(
            drawing
                .borrow()
                .get_grid()
                .unwrap()
                .get_color(4, 4)
                .unwrap(),
            (0, 0)
        );
        assert_eq!(canvas[3][3], (1, 0));
        assert_eq!(canvas[3][2], (1, 0));
        assert_eq!(canvas[2][1], (1, 0));
        assert_eq!(canvas[1][0], (1, 0));

        let mouse_pos = (1, 0);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(canvas[3][3], (1, 0));
        assert_eq!(canvas[2][3], (1, 0));
        assert_eq!(canvas[1][2], (1, 0));
        assert_eq!(canvas[0][1], (1, 0));

        let mouse_pos = (7, 0);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(canvas[3][5], (1, 0));
        assert_eq!(canvas[2][5], (1, 0));
        assert_eq!(canvas[1][6], (1, 0));
        assert_eq!(canvas[0][7], (1, 0));

        let mouse_pos = (8, 1);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(canvas[3][5], (1, 0));
        assert_eq!(canvas[2][6], (1, 0));
        assert_eq!(canvas[2][7], (1, 0));
        assert_eq!(canvas[1][8], (1, 0));

        let mouse_pos = (8, 7);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(canvas[5][5], (1, 0));
        assert_eq!(canvas[6][6], (1, 0));
        assert_eq!(canvas[6][7], (1, 0));
        assert_eq!(canvas[7][8], (1, 0));

        let mouse_pos = (7, 8);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(canvas[5][5], (1, 0));
        assert_eq!(canvas[6][6], (1, 0));
        assert_eq!(canvas[7][6], (1, 0));
        assert_eq!(canvas[8][7], (1, 0));

        let mouse_pos = (1, 8);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(canvas[5][3], (1, 0));
        assert_eq!(canvas[6][2], (1, 0));
        assert_eq!(canvas[7][2], (1, 0));
        assert_eq!(canvas[8][1], (1, 0));

        let mouse_pos = (0, 7);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[4][4], (1, 0));
        assert_eq!(canvas[5][3], (1, 0));
        assert_eq!(canvas[5][2], (1, 0));
        assert_eq!(canvas[6][1], (1, 0));
        assert_eq!(canvas[7][0], (1, 0));

        // 水平な線
        let mouse_pos = (8, 4);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        for i in 4..=8 {
            assert_eq!(canvas[4][i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, 4)
                    .unwrap(),
                (0, 0)
            );
        }

        let mouse_pos = (0, 4);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        for i in 0..=4 {
            assert_eq!(canvas[4][i], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, 4)
                    .unwrap(),
                (0, 0)
            );
        }

        // 垂直な線
        let mouse_pos = (4, 0);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        for i in 0..=4 {
            assert_eq!(canvas[i][4], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(4, i)
                    .unwrap(),
                (0, 0)
            );
        }

        let mouse_pos = (4, 8);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        for i in 4..=8 {
            assert_eq!(canvas[i][4], (1, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(4, i)
                    .unwrap(),
                (0, 0)
            );
        }
    }

    #[test]
    fn line_not_mouse_down_test() {
        let mut line = Line::new();
        let mut canvas = vec![vec![(0, 0).into(); CANVAS_EDGE_SIZE]; CANVAS_EDGE_SIZE];
        let canvas_size = (CANVAS_EDGE_SIZE, CANVAS_EDGE_SIZE);
        let drawing = Rc::new(RefCell::new(DrawingMock::new()));

        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .add_color(ODColor::new(0, 0, 0))
            .is_ok());
        assert!(drawing
            .borrow()
            .get_palette()
            .borrow_mut()
            .select_color((1, 0).into())
            .is_ok());

        assert!(drawing
            .borrow()
            .get_active_layer()
            .borrow_mut()
            .set_grid_width(CANVAS_EDGE_SIZE)
            .is_ok());
        assert!(drawing
            .borrow()
            .get_active_layer()
            .borrow_mut()
            .set_grid_height(CANVAS_EDGE_SIZE)
            .is_ok());

        // マウスダウンしていない状態でドラッグ
        let mouse_pos = (0, 0);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        assert_eq!(canvas[0][0], (0, 0));
        assert_eq!(
            drawing
                .borrow()
                .get_grid()
                .unwrap()
                .get_color(0, 0)
                .unwrap(),
            (0, 0)
        );

        let mouse_pos = (5, 5);
        assert!(line
            .on_mouse_drag(&mut canvas, &canvas_size, drawing.clone(), &mouse_pos)
            .is_ok());
        for i in 0..10 {
            assert_eq!(canvas[i][i], (0, 0));
            assert_eq!(
                drawing
                    .borrow()
                    .get_grid()
                    .unwrap()
                    .get_color(i, i)
                    .unwrap(),
                (0, 0)
            );
        }
    }
}
