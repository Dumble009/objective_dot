...
impl Bitmap {
    pub fn from_drawing(
        ...
        let width = grid.get_grid_width() * pixel_per_dot;
        let height = grid.get_grid_height() * pixel_per_dot;

        let mut pixels = Vec::with_capacity(width * height * 4);

        for y in 0..height {
            for x in 0..width {
                let color_index = grid.get_color(x / pixel_per_dot, y / pixel_per_dot)?;
